...
exports.checkOneEscape = (req, res, next) => {
  ...
  Escape.findOne({}, (err, escape) => {
    if (err) { return next(err); }
    if (!escape) {
      return res.status(401).send({message: false, message: 'ERROR'});
    }
...
exports.getReferralLink = (req, res, next) => {
  ...
  if (!['member', 'admin'].includes(role)){
    return res.status(422).send( {message: status.ROLE_INCORRECT.message, status: status.ROLE_INCORRECT.code} );
  }
  crypto.randomBytes(48, function(err, buffer) {
    if (err) { return next(err); }
    const token = buffer.toString('hex');
    Referral.findOne({referral: token}, (err, existingReferralLink) => {
      if (existingReferralLink) {
        return res.status(422).send( {message: status.TRY_AGAIN.message, status: status.TRY_AGAIN.code} );
      }
      if (err) { return next(err); }
