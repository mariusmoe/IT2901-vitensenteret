...
@Component({
  ...
  trigger('jumpRotate', [
    ...
    state('active', style({transform: 'scale(1.1,1.1)'})),
    transition('inactive => active', animate('500ms', keyframes([
      style({transform: 'scale(0.9,0.9)', offset: 0}),
      style({transform: 'scale(1.4,1.4) rotate(-15deg)', offset: 0.50}),
      style({transform: 'scale(0.9,0.9) rotate(5deg)', offset: 0.75}),
      style({transform: 'scale(1.1,1.1)', offset: 1.0})
    ]))),
